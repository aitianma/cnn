...
for k in range(10000):
	...
	p=np.zeros([25,25])
		...
		for j in range(25):
			x_b=x[i:i+4,j:j+4]
			p[i][j]=np.exp(x_b.reshape(1,16)*w_cov.reshape(16,1)+b_cov).sum()
	fc=np.zeros([5,5])
	for i in range(5):
		for j in range(5):
			fc[i][j]=p[i:i*5+5,j:j*5+5].max()
	...
	for i in range(5):
		for j in range(5):
			dy_p[i:i+5,j:j+5]=dy_fc.reshape([5,5])[i][j]*p[i:i+5,j:j+5]
	   
	w_fc=w_fc+rate*np.matmul(np.reshape(fc,[25,1]),dy)	  
	for i in range(25):
		for j in range(25):
