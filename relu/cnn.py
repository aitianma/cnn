...
for k in range(5000):
	...
	w_cov1=w_cov1+rate1*FZ(signal.convolve2d(cov,FZ(dy_fc1*relu_der(cov1)),"valid"))
	w_cov1=w_cov1+rate1*FZ(signal.convolve2d(cov_1,FZ(dy_fc2*relu_der(cov1_1)),"valid"))	

	w_cov[0]=w_cov[0]+rate2*FZ(signal.convolve2d(x,FZ(dy_cov1*relu_der(cov)),"valid"))
	w_cov[1]=w_cov[1]+rate2*FZ(signal.convolve2d(x,FZ(dy_cov1*relu_der(cov_1)),"valid"))	
