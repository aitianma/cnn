...
for k in range(5000):
	...
	cov=signal.convolve2d(x,FZ(w_cov),"valid")
	cov=1/(1+np.exp(-cov))
	...
	cov1=signal.convolve2d(p,FZ(w_cov1),"valid")
	...
	dy_p=signal.convolve2d(dy_cov1,w_cov1,"full")
	#dy_p=dy_p[::-1]
	p_rot_180=FZ(p)
#	for i in range(8):
#		for j in range(8):	
#			w_cov1=w_cov1+rate1*p_rot_180[i:i+5,j:j+5]*dy_cov1[i][j]
	...
	w_cov1=w_cov1+rate1*signal.convolve2d(dy_cov1*cov1*(1-cov1),p,"valid")
	...
	w_cov=w_cov+rate2*signal.convolve2d(dy_cov*cov*(1-cov),FZ(x),"valid")
